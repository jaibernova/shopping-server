...
export default {
    ...
    async calculateFinalPrice(userObj, shippingAddress, save = false) {
        try {
            ...
            if (shoppingCart.subTotalPrice.amount<150000) {
                shippingPriceInUSD = 6000;                
            }
    ...
    async completeCheckoutUsingStripePaymentInstant(checkoutId, paymentToken, userObj) {
        let result = {};
        try {
            // Make sure at least the required params are passed
            if (!(checkoutId && paymentToken)) {
            ...
            logger.error("Error in completeCheckoutUsingStripePaymentInstant Service", { meta: err });
            result = { httpStatus: httpStatus.BAD_REQUEST, status: "failed", errorDetails: err };
            return result;
        }
    },
